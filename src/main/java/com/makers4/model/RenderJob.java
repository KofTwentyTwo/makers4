...
@QMetaDataProducingEntity(
   ...
   tableMetaDataCustomizer = RenderJob.TableMetaDataCustomizer.class,
   producePossibleValueSource = true,
   childTables = {
      @ChildTable(
         joinFieldName = "renderJobId",
         childTableEntityClass = RenderArtifact.class,
         childJoin = @ChildJoin(enabled = true),
         childRecordListWidget = @ChildRecordListWidget(enabled = true, label = "Render Artifacts", maxRows = 20)
      )
   }
)
